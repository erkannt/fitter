from config import MAX_HR, ZONE_BOUNDS, hr_zone_index


def test_hr_zone_index_below_z1() -> None:
    assert hr_zone_index(0) == 0


def test_hr_zone_index_boundary() -> None:
    z1_top = int(ZONE_BOUNDS[0][1])
    assert hr_zone_index(z1_top - 1) == 0
    assert hr_zone_index(z1_top) == 1


def test_hr_zone_index_max() -> None:
    assert hr_zone_index(MAX_HR - 1) == 4
    assert hr_zone_index(MAX_HR) == 4


def test_hr_zone_index_mid_z3() -> None:
...
def test_hr_zone_index_at_z1_boundary() -> None:
